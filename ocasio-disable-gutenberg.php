<?php
/**
 * Plugin Name: Ocasio Disable Gutenberg
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-disable-gutenberg/
 * Description: Restores the Classic Editor cleanly and prevents block editor CSS stylesheets from loading on the front end.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-disable-gutenberg
 */
...
define('OCASIO_DG_VERSION', '1.0.0');
define('OCASIO_DG_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_DG_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_dg_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_dg_activate() {
	if (get_option('ocasio_dg_enabled') === false) {
		$legacy = get_option('ko_dg_enabled', 1);
		update_option('ocasio_dg_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_dg_activate');

/**
 * 2. Core Logic: Disable Block Editor & Restore Classic Editor
 */
function ocasio_dg_is_enabled() {
	$enabled = get_option('ocasio_dg_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_dg_enabled', 1);
	}
...
add_filter('use_block_editor_for_post', function ($use_block_editor) {
	return ocasio_dg_is_enabled() ? false : $use_block_editor;
}, 10);

// Disable Gutenberg for post types
add_filter('use_block_editor_for_post_type', function ($use_block_editor) {
	return ocasio_dg_is_enabled() ? false : $use_block_editor;
}, 10);

// Dequeue block editor stylesheets on front end
add_action('wp_enqueue_scripts', function () {
	if (!ocasio_dg_is_enabled()) {
		return;
	}
 ...
 */
function ocasio_dg_register_settings() {
	register_setting('ocasio_dg_settings_group', 'ocasio_dg_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_dg_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_dg_register_menu() {
...
add_action('admin_menu', 'ocasio_dg_register_menu');

/**
 * 5. Action Links on Plugins Screen
 */
function ocasio_dg_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-plugins-main')) . '">' . esc_html__('Dashboard', 'ocasio-disable-gutenberg') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_dg_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_dg_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-dg-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_DG_VERSION);
		wp_enqueue_script('ocasio-dg-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_DG_VERSION, true);
		wp_localize_script('ocasio-dg-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-dg-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_dg_admin_assets');
